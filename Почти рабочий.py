    ...
    def run(self):
        try:
            ...
            for idx in range(0, len(route_points), 2):
                ...
                ground_alt = self.get_elevation_at(lat, lon)
                candidates.append((lat, lon, ground_alt, dist_from_start))
            ...
            # Сортируем кандидатов по высоте (от самой высокой)
            ...
            for lat, lon, ground_alt, dist_from_start in candidates[:50]:
                if not self._is_running:
                    self.finished.emit(None)
                    return
                ...
                for shadow_idx in range(0, len(first_shadow_zone), max(1, len(first_shadow_zone) // check_count)):
                    ...
                    if self.check_line_of_sight_fast(lat, lon, ground_alt, 
                ...
                # Оценка: высота + покрытие
                height_factor = (ground_alt - start_abs) / 100
...
class MapWidget(QWidget):
    ...
    def check_visibility_with_relay(self, operator_antenna_height=2, relay_antenna_height=2):
        """Проверка видимости с учетом ретранслятора"""
        if self.start_point is None or len(self.waypoints) < 2:
            return [], []
        ...
        # Если ретранслятор установлен - проверяем с учетом ретранслятора
        ...
        for i in range(len(flight_rel)):
            ...
            # Если точка до ретранслятора - проверяем от старта
            if dist_from_start <= dist_to_relay:
                has_los = self.check_line_of_sight(start_lat, start_lon, start_abs, 
                                                  lat, lon, alt, operator_antenna_height)
                if not has_los:
                    shadow_indices.append(i)
            else:
                # Точка после ретранслятора - проверяем от ретранслятора
                has_los = self.check_line_of_sight(relay_lat, relay_lon, relay_abs, 
                                                  lat, lon, alt, relay_antenna_height)
                if not has_los:
    ...
    def paintEvent(self, event):
        ...
        if self.relay_point:
            ...
            relay_alt = self.get_elevation_at(self.relay_point[0], self.relay_point[1])
    ...
    def paintEvent(self, event):
        ...
        for i in range(len(flight_points) - 1):
            if i in self.shadow_after_relay or (i+1) in self.shadow_after_relay:
                # Если это не было тенью ДО ретранслятора, рисуем фиолетовым
...
class RoutePlanner(QMainWindow):
    ...
    def update_profile(self):
        ...
        # Используем новый метод проверки видимости с учетом ретранслятора
        ...
        for i in range(len(flight_rel)):
            if i < len(distances):
                lat, lon = self.get_point_coordinates(i)
                alt = start_abs + flight_rel[i]
    ...
    def get_point_coordinates(self, index):
        ...
        total_segments = len(self.waypoints) - 1
        segment_idx = min(index * total_segments // max(1, 100), total_segments - 1)
        
        if segment_idx < len(self.waypoints) - 1:
            lat1, lon1, alt1 = self.waypoints[segment_idx]
            lat2, lon2, alt2 = self.waypoints[segment_idx + 1]
            
            points_in_segment = 100 // total_segments
            if points_in_segment > 0:
                t = (index % points_in_segment) / points_in_segment
            else:
                t = 0
    ...
    def check_route(self):
        ...
        if alt_violations or shadow_indices or shadow_after_relay:
            ...
            if self.relay_point and not shadow_indices and not shadow_after_relay:
            ...
            if self.relay_point:
                msg += f"📡 Ретранслятор обеспечивает полное покрытие!\n"
    ...
    def on_relay_search_finished(self, relay_pos):
        try:
            ...
            if relay_pos:
                ...
                relay_abs_alt = self.map_widget.get_elevation_at(relay_pos[0], relay_pos[1])
                ...
                if shadow_after:
                    QMessageBox.information(self, "Ретранслятор установлен",
                                          f"✅ Ретранслятор установлен НА НАИВЫСШЕЙ ТОЧКЕ!\n\n"
                                          f"Расстояние от старта: {self.relay_position_km:.1f} км\n"
                                          f"Высота: {relay_rel_alt:.0f} м относительно старта\n"
                                          f"Широта: {relay_pos[0]:.5f}\n"
                                          f"Долгота: {relay_pos[1]:.5f}\n\n"
                    ...
                    QMessageBox.information(self, "Ретранслятор установлен",
                                          f"✅ Ретранслятор успешно установлен НА НАИВЫСШЕЙ ТОЧКЕ!\n\n"
                                          f"Расстояние от старта: {self.relay_position_km:.1f} км\n"
                                          f"Высота: {relay_rel_alt:.0f} м относительно старта\n"
                                          f"Широта: {relay_pos[0]:.5f}\n"
                                          f"Долгота: {relay_pos[1]:.5f}\n\n"
